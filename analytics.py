"""
PROJECT HOPE v3.0 - Performance Analytics
Tracks every trade and calculates pro-level metrics
"""
import math
...
class Analytics:
    ...
    def get_full_report(self):
        ...
        max_win_streak, max_loss_streak, current_streak, streak_type = self._calc_streaks()
        ...
        for t in self.trade_history:
            ...
            if st not in setup_stats:
                setup_stats[st] = {'wins': 0, 'losses': 0, 'pnl': 0}
            if t['pnl'] > 0:
        ...
        return {
            ...
            'monthly': monthly,
            'spread_stats': self._strat_stats(spread_trades, 'Credit Spreads'),
            'directional_stats': self._strat_stats(dir_trades, 'Directional'),
    ...
    def _calc_streaks(self):
        ...
        for t in self.trade_history:
            if t['pnl'] > 0:
                cur = cur + 1 if last == 'w' else 1
                last = 'w'; ct = 'win'; mw = max(mw, cur)
            else:
                cur = cur + 1 if last == 'l' else 1
                last = 'l'; ct = 'loss'; ml = max(ml, cur)
        return mw, ml, cur, ct
    ...
    def _strat_stats(self, trades, name):
    ...
    def _empty_report(self):
        return {
            ...
            'monthly':[],'spread_stats':{'name':'Credit Spreads','trades':0,'win_rate':0,'pnl':0},
